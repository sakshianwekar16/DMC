	...
	if (angle < 0x8000) {
		...
		if (angle < 0xd555) {
			if (angle < 0xaaaa) {
				...
				MotorRun.PDC3Latch = t1 + t2 + half_t0;
			} else {
				MotorRun.PDC1Latch = t2 + half_t0;
				MotorRun.PDC2Latch = half_t0;
				MotorRun.PDC3Latch = t1 + t2 + half_t0;
			}
		} else {
			MotorRun.PDC1Latch = t1 + t2 + half_t0;
...
//update_PDCValues(PDC1Latch,PDC2Latch,PDC3Latch);
